...
function colorGraph(graph, colors) {
  ...
  for(let i = 0; i < graph.length; i++) {
    ...
    [...(graph[i].neighbors)].forEach(neighbor => {
      if (graph[i] == neighbor) {
        throw 'error';
      }
    })
    
    let neighborColors = [...(graph[i].neighbors)].map(neighbor => neighbor.color);
    
    let validColor = colors.filter(color => !neighborColors.includes(color))
    
    if (validColor.length > 0) {
      graph[i].color = validColor[0]
    }
  }
  // console.log(graph)
}
